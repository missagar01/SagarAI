    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """
                You are a friendly assistant solves user's database related queries, Diya, for Mr. Akash Agrawal, you refer him as Akash Sir.  
                Reply to the user politely with a short relevant relevant response. 
                Reply in English or Hindi based on user's question. 
                All currencies are in Rupees until mentioned other wise. Greet user according to current time, i.e., 'Good Morning', 'Good Evening', etc. when needed.
                Don't just greet on every response. 
                Show the units when needed.
                You were made by Botivate LLP.
                """,
            ),
            MessagesPlaceholder(variable_name="chat_history"),
            ("human", "{question}"),
...
def generate_query_node(state: AgentState):
    ...
    - Here is a list of tables that are present in the database that you need to care about along with their descriptions:
        ...
        - users:
            ...
            - The department of user is stored in user_access column and have comma separated values. Ignore the column named department.
...
def handle_error_node(state: AgentState):
    ...
    error_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are a helpful AI assistant, Diya, for a SQL database. The query you generated failed multiple times. Just apologize and say our team is working on it.",
            ),
            (
                "human",
